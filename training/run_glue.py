...
# This file is adapted from https://github.com/twinkle0331/BERT-similarity
...
# added. merges two state dictionaries with w weight
def get_merged_state_dict(state_dict_1, state_dict_2, w=0.5, save_both=False, unmerge=None):
        """
...
def main():
    ...
    if model_args.not_state_dict:
        model = torch.load(model_args.model_name_or_path)
    elif model_args.merged:
        model1 = torch.load(model_args.model1_path)
        model2 = torch.load(model_args.model2_path)
        model_placeholder = BertForMaskedLM.from_pretrained('google/multiberts-seed_0')
        model_placeholder.load_state_dict(get_merged_state_dict(model1,model2))
        ...
        model.bert.embeddings = model_placeholder.bert.embeddings
        model.bert.encoder = model_placeholder.bert.encoder
    else:
        model = BertForSequenceClassification.from_pretrained(
